...
class MyPatientFor3DUNet(Dataset):
    def __init__(self, in_channels=1, out_channels=1, step='train', data_type='float32'):
        ...
        if step == 'train':
            ...
            self.folder_path_mask = path + "/val/mask/"
            self.folder_path_mask_ = path + "/val/mask_/"
            self.folder_path_PET = path + "/val/image/"
            self.mask_list = os.listdir(self.folder_path_mask)
            self.mask_list = natsorted(self.mask_list)
            self.data_len = len(self.mask_list)
    ...
    def __getitem__(self, idx):
        raw = cv2.imread(self.folder_path_PET + self.mask_list[idx], 0)
        ...
        # mask_ = cv2.resize(mask,(256,256),interpolation=cv2.INTER_NEAREST)
