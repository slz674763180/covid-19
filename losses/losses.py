...
SUPPORTED_LOSSES = ['ce', 'bce', 'wce', 'pce', 'dice', 'gdl', 'EntropyLoss', 'focal']
...
class IgnoreIndexLossWrapper:
    ...
    def __init__(self, loss_criterion, ignore_index=-1):
        if hasattr(loss_criterion, 'ignore_index'):
            raise RuntimeError("Cannot wrap {type(loss_criterion)}. Use 'ignore_index' attribute instead")
...
def get_loss_criterion(loss_str, weight=None, ignore_index=None):
    ...
    """
    assert loss_str in SUPPORTED_LOSSES, 'Invalid loss string: {}'.format(loss_str)
    if loss_str == 'bce':
